...
namespace CODE.Framework.Wpf.Layout
{
    ...
    public class FlowForm : Panel
    {
        ...
        protected virtual Size CalculateLabelSize(string label, DependencyObject element)
        {
            ...
            var ft = new FormattedText(label, CultureInfo.CurrentUICulture, FlowDirection.LeftToRight, typeface, size, brush, pixelsPerDip) { Trimming = TextTrimming.WordEllipsis, TextAlignment = LabelTextAlignment };
            ...
            if (_currentRows != null)
            {
                ...
                foreach (var row in _currentRows)
                    foreach (var control in row.Elements.Where(element => element.IsAutoGeneratedLabel))
                    {
                        ...
                        var ft = new FormattedText(control.Label, CultureInfo.CurrentUICulture, FlowDirection.LeftToRight, typeface, control.FontSize, control.ForegroundBrush, pixelsPerDip) { Trimming = TextTrimming.WordEllipsis, TextAlignment = LabelTextAlignment };
